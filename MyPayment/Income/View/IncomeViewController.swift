...
class IncomeViewController: UIViewController {
    ...
    var viewModel: IncomeViewControllerViewModelType? = IncomeViewModel()
    ...
    override func viewDidLoad() {
        ...
        self.popUpCollectionView.register(UINib(nibName: "IncomeCollectionViewCell", bundle: nil), forCellWithReuseIdentifier: "IncomeImageCell")
...
//MARK: Save item in Realm
    ...
    func saveItem() {
        ...
        viewModel?.incomeData.save(item: item)
        viewModel?.popUpSave()
        datePicker.date = .now
...
//MARK: Button actions
    ...
    @IBAction func popUpChangeButtonAction(_ sender: Any) {
        guard let viewModel = viewModel else { return }
        for i in viewModel.incomeData.getItems() {
            if i.date == viewModel.itemDate && i.date?.seconds == viewModel.itemSeconds {
                viewModel.incomeData.remove(item: i)
               saveItem()
            }
        }
    }
...
extension IncomeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return viewModel?.collectionViewNumberOfItemsInSection() ?? 0
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = popUpCollectionView.dequeueReusableCell(withReuseIdentifier: "IncomeImageCell", for: indexPath) as? IncomeCollectionViewCell
